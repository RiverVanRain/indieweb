...
class MentionClient {
	...
	*/
	public static function sendPingbackToEndpoint($endpoint, $source, $target) {
		...
		$payload = static::xmlrpc_encode_request('pingback.ping', [$source,  $target]);

		$response = static::_post($endpoint, $payload, [
	...
	*/
	protected function _findWebmentionEndpointInHTML($body, $targetURL = false) {
		...
		if (preg_match('/<(?:link|a)[ ]+href="([^"]*)"[ ]+rel="[^" ]* ?webmention ?[^" ]*"[ ]*\/?>/i', $body, $match) || preg_match('/<(?:link|a)[ ]+rel="[^" ]* ?webmention ?[^" ]*"[ ]+href="([^"]*)"[ ]*\/?>/i', $body, $match)) {
			$endpoint = $match[1];
		}
	...
	*/
	public static function sendWebmentionToEndpoint($endpoint, $source, $target, $additional = []) {
		...
		return static::_post($endpoint, $payload, [
	...
	*/
	public function sendWebmention($sourceURL, $targetURL, $additional = []) {
		...
		if ($webmentionServer) {
			...
			self::_debug("sendWebmention: No found webmentionServer");
			return false;
		}
	}
	...
	*/
	public static function findOutgoingLinks($input) {
		...
		if (is_string($input)) {
			...
			$links = array_merge(array_unique($matches[1]), self::findLinksInText($input));
		
			return array_unique($links);
		} else if (is_array($input) && array_key_exists('items', $input) && array_key_exists(0, $input['items'])) {
			$links = [];
			...
			self::_debug("findOutgoingLinks: No links");
			return [];
		}
	}
	...
	*/
	public function sendMentions($sourceURL, $sourceBody = false) {
		...
		self::_debug("sendMentions: totalAccepted - " . $totalAccepted);
	...
	*/
	public function sendFirstSupportedMention($source, $target) {
		...
		if	($this->discoverWebmentionEndpoint($target)) {
			...
			if ($result && ($result['code'] == 200 || $result['code'] == 201 || $result['code'] == 202)) {
				$accepted = 'webmention';
		...
		self::_debug("sendFirstSupportedMention: " . $accepted);
	...
	*/
	protected static function _head($url, $headers = []) {
		...
		if (self::$_proxy) {
			curl_setopt($ch, CURLOPT_PROXY, self::$_proxy);
		}
